...
const SingleServerMetrics: FC = () => {
    ...
    function getTotalCount4xx(data: IMetrics | undefined, sectionKey:string): number {
        ...
        return entries?.reduce((total: number, entry: IErrorCount) => {
    ...
    return (
        <>
            <div className='single-item'>
                ...
                <div key={i}>
                <span key={err.count4xx}>4xx Error: {getTotalCount4xx(data, 'h1')}</span>
                <span key={err.count5xx}>5xx Error: {err.count5xx}</span>
                </div>
                )}
