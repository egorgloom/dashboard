...
const OverviewPage: FC<IOverviewPage> = () => {
    ...
    //   const historicalData = [data?.[0]?.location?.historicalData?.[selectedPeriod]];

const historicalData = data ? [data?.[0]?.location?.historicalData?.[selectedPeriod]] : null;
    ...
    return (
        <>
            <div className='wrapper'>
                ...
                {historicalData?.map((elem: any) => <div>{elem.cpu}</div>)}
